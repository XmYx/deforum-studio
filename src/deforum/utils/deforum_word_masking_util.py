...
def find_clipseg(root):
    ...
    for basedir in root.basedirs:
        src_basedirs.append(
            os.path.join(os.path.sep.join(os.path.abspath(__file__).split(os.path.sep)[:-2]), 'deforum_helpers', 'src'))
...
def get_word_mask(root, frame, word_mask):
    ...
    resized_mask = interpolate(mask, size=(frame.size[1], frame.size[0]),
                               mode='bicubic').squeeze()  # rescale mask back to the target resolution
