...
def vid2frames(video_path, video_in_frame_path, n=1, overwrite=True, extract_from_frame=0, extract_to_frame=-1,
               out_img_format='jpg', numeric_files_output=False):
    ...
    if n < 1:
        n = 1  # HACK Gradio interface does not currently allow min/max in gr.Number(...)
    ...
    if is_vid_path_valid(video_path):
        ...
        if n >= frame_count:
            raise RuntimeError(
                'Skipping more frames than input video contains. extract_nth_frames larger than input frames')
        ...
        if len(input_content) == 0:
            ...
            max_workers = int(
                max(1, int(cpu_count / 2)) - 1)  # set max threads to cpu cores halved, minus 1. minimum is 1
...
def is_vid_path_valid(video_path):
    ...
    if video_path.startswith('http://') or video_path.startswith('https://'):
        ...
        if extension not in file_formats:
            raise ValueError(
                "Video file format '{}' not supported. Supported formats are: {}".format(extension, file_formats))
    else:
...
def ffmpeg_stitch_video(ffmpeg_location=None, fps=None, outmp4_path=None, stitch_from_frame=0, stitch_to_frame=None,
    ...
    try:
        cmd = [ffmpeg_location, '-y', '-r', str(float(fps)), '-start_number', str(stitch_from_frame), '-i', imgs_path,
               '-frames:v', str(stitch_to_frame), '-c:v', 'libx264', '-vf', f'fps={float(fps)}', '-pix_fmt', 'yuv420p',
               '-crf', str(crf), '-preset', preset, '-pattern_type', 'sequence', '-vcodec',
               'png' if imgs_path[0].find('.png') != -1 else 'libx264', outmp4_path]
...
def check_and_download_gifski(models_folder, current_user_os):
    ...
    if not os.path.exists(file_path):
        ...
        if checksum(file_path) != checksum_value:
            raise Exception(
                f"Error while downloading {file_name}. Please download from: {download_url} and place in: {models_folder}")
...
def make_gifski_gif(imgs_raw_path, imgs_batch_id, fps, models_folder, current_user_os):
    ...
    if current_user_os == "Linux":
        ...
        input_img_files = [os.path.join(imgs_raw_path, file) for file in
                           sorted(glob.glob(os.path.join(imgs_raw_path, input_img_pattern)))]
        ...
        cmd = [gifski_location, '-o', final_gif_path, input_img_pattern_for_gifski, '--fps', str(fps), '--quality',
               str(95)]
    ...
    try:
        subprocess.run(cmd, capture_output=True, check=True, text=True,
                       cwd=(models_folder if current_user_os == 'Mac' else None))
        print("\r" + " " * len(msg_to_print), end="", flush=True)
        print(f"\r{msg_to_print}", flush=True)
...
def delete_matching_frames(from_folder, img_batch_id):
    return sum(1 for f in os.listdir(from_folder) if
               get_matching_frame(f, img_batch_id) and os.remove(os.path.join(from_folder, f)) is None)
...
def get_matching_frame(f, img_batch_id=None):
    return ('png' in f or 'jpg' in f) and '-' not in f and '_depth_' not in f and (
