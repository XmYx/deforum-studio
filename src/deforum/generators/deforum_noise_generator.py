...
# ChatGPT Rewrite without Lambda to adhere with PEP 731
...
def rand_perlin_2d(shape, res, fade=lambda t: 6 * t ** 5 - 15 * t ** 4 + 10 * t ** 3):
    ...
    def dot(grad, shift):
        return (torch.stack((grid[:shape[0], :shape[1], 0] + shift[0], grid[:shape[0], :shape[1], 1] + shift[1]),
...
def add_noise(sample, noise_amt: float, seed: int, noise_type: str, noise_args, noise_mask=None, invert_mask=False):
    ...
    if noise_type == 'perlin':
        ...
        noise = noise * ((rand_perlin_2d_octaves(sample2dshape, (int(noise_args[0]), int(noise_args[1])),
                                                 octaves=noise_args[2], persistence=noise_args[3]) + torch.ones(
            sample2dshape)) / 2)
