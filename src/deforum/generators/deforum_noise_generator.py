...
#
#     grid = torch.stack(
#         torch.meshgrid(torch.arange(0, res[0], delta[0]),
...
#
#     tile_grads = lambda slice1, slice2: gradients[slice1[0]:slice1[1],
#     slice2[0]:slice2[1]].repeat_interleave(d[0],
#                                                                                                               0).repeat_interleave(
#         d[1], 1)
#     grid = grid.to('cuda')
...
#         noise += amplitude * rand_perlin_2d(shape, (frequency * res[0], frequency * res[1])).to(device)
#         frequency *= 2
...
#     noise = torch.randn((sample.shape[2], perlin_w, perlin_h), generator=deforum_noise_gen, device='cuda')  # White noise
...
#         print(noise.device)
#
#         noise = noise * ((rand_perlin_2d_octaves(sample2dshape, (int(noise_args[0]), int(noise_args[1])),
#                                                  octaves=noise_args[2], persistence=noise_args[3], device='cuda').to('cuda') + torch.ones(
#             sample2dshape).to('cuda')) / 2)
#         noise = interpolate(noise.unsqueeze(1), size=(sample.shape[0], sample.shape[1])).squeeze(
#             1)  # rescale perlin back to the target resolution
